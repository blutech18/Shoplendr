...
import 'homepage_copy2_copy_model.dart';
export 'homepage_copy2_copy_model.dart';
...
class HomepageCopy2CopyWidget extends StatefulWidget {
  ...
  static String routeName = 'HomepageCopy2Copy';
  static String routePath = '/homepageCopy2Copy';
...
class _HomepageCopy2CopyWidgetState extends State<HomepageCopy2CopyWidget> {
    ...
    return StreamBuilder<UsersRecord>(
      ...
      builder: (context, snapshot) {
          ...
          child: Column(
            ...
            children: [
              if (!isProfileComplete && snapshot.hasData)
                _buildProfileIncompleteBanner(snapshot.data!),
              Padding(
              ...
              Expanded(
                child: Padding(
                  ...
                  builder: (context, snapshot) {
                    ...
                    // Fetch confirmed purchase requests to filter out sold items
                    return StreamBuilder<List<PurchaseRequestsRecord>>(
                      ...
                      builder: (context, purchaseSnapshot) {
                        // Show loading while fetching purchase requests
                        ...
                        // Get list of sold product references
                        final soldProductRefs = purchaseSnapshot.data!
                        ...
                        final availableProducts = allProducts
                            .where((product) => !soldProductRefs.contains(product.reference))
                            .toList();
        ...
        bottomNavigationBar: BottomNavBar(
          currentRoute: HomepageCopy2CopyWidget.routeName,
  ...
  String _getProfileIncompleteReason(UsersRecord user) {
    if (user.studentIdVerified != true) {
      return 'Your student ID verification is pending approval from admin.';
    }
    if (user.displayName.isEmpty) {
      return 'Please complete your name in the profile.';
    }
    if (user.phoneNumber.isEmpty) {
      return 'Please add your GCash number in the profile.';
    }
    if (user.address.isEmpty) {
      return 'Please add your address in the profile.';
    }
    return 'Please complete your profile to access all features.';
  }
  
  Widget _buildProfileIncompleteBanner(UsersRecord user) {
    final reason = _getProfileIncompleteReason(user);
